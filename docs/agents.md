...
4. **Ciclo Estricto de Testing (Mandatorio)**: Una vez implementada una solución, el QA Agent DEBE trazar mentalmente el flujo completo de la interacción o testearlo lógicamente. **Si la solución no garantiza un 100% de éxito, se debe cuestionar la decisión estructuralmente, replantear la solución y proponer un camino nuevo.** Está prohibido dar por válida una tarea simplemente porque "el código se ha escrito".
5. **Adaptabilidad**: El Orquestador tiene autonomía para ascender (escalar) al modelo Opus/Pro-HIGH si el modelo Sonnet/Flash se topa con un roadblock complejo.
